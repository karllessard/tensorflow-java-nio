 ...
 * <li>Use of generics allows to work directly with boxed types as well, which does not require
 * explicit buffer types as with the standard JDK buffers.
 * </ul>
 * It is important to note that, as opposed to the JDK buffers, there is no guarantee that the
 * buffers are linear, specially when dealing with non-primitive types or large buffers.
 *
...
public interface DataBuffer<T> {
   ...
   * can hold. For an integer buffer, it is equal to the number of integers, therefore the size in
   * bytes of this buffer is {@code capacity() * Integer.BYTES}.
   *
   ...
   *
   * @param limit the limit of the buffer copy; must be non-negative and no larger than this
   * buffer's capacity
   ...
   * <p>
   * Each buffer as an internal cursor that automatically moves forward after a read ({@code get})
   * or write ({@code put}) operation. The position indicates the current index of this cursor.
   */
  long position();
   ...
   *
   * Writes the given value into this buffer at the current position, and then increments the
   * position.
   *
   ...
   * <p>
   * This method transfers the values remaining in the given source buffer into this buffer. If
   * there are more values remaining in the source buffer than in this buffer, that is, if {@code
   * src.remaining() > remaining()}, then no values are transferred and a BufferOverflowException is
   * thrown.
   * <p>
   * Otherwise, this method copies {@code n = src.remaining()} values from the given buffer into
   * this buffer, starting at each buffer's current position. The positions of both buffers are then
   ...
   * will be visible in the new buffer, and vice versa; the two buffers' position and limit values
   * will be independent.
   * <p>
   * The new buffer's capacity, limit and position will be identical to those of this buffer. The
   ...
   * and limit values will be independent.
   * <p>
   * The new buffer's position will be zero, its capacity and its limit will be the number of chars
   * remaining in this buffer. The new buffer will be read-only if, and only if, this buffer is
   * read-only.
   *
   * @return the new buffer
   */
